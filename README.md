...
This project automates the deployment of a bare metal Kubernetes cluster using Flatcar Container Linux and Kubeadm.
...
6. **Boot Machines**:
    ...
    * **Note**: The cluster will come up in a `NotReady` state initially because no CNI is installed.

7. **Retrieve Kubeconfig**:
    Once the control plane node responds to SSH (or is pingable), retrieve the admin kubeconfig:
