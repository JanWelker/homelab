...
Alphabetical, with the namespace it lands in.
...
| backup | `backup` | The CSI snapshot controller, Velero's buckets and an etcd snapshot CronJob — see [Backups & Recovery](../operations/backups.md) |
| [cert-manager](cert-manager.md) | `cert-manager` | Let's Encrypt wildcards over a Route53 DNS-01 challenge |
...
| [gateway-api](gateway-api.md) | `gateway-system` | The two Gateways and the HTTP-to-HTTPS redirect |
...
| [logging](logging.md) | `logging` | Loki and Grafana Alloy, for container, journal and audit logs |
| [metrics-server](metrics-server.md) | `kube-system` | The `metrics.k8s.io` API behind `kubectl top` and every HPA |
| [monitoring](monitoring.md) | `monitoring` | Prometheus, Grafana, Alertmanager, node-exporter, kube-state-metrics |
| [openbao](openbao.md) | `openbao` | Cluster-wide secret store |
| [rook-ceph](rook-ceph.md) | `rook-ceph` | Replicated block storage and an S3 object store |
| [tetragon](tetragon.md) | `kube-system` | Runtime detection: execs, credential changes and sensitive file reads from the kernel |
| [security policies](security-policies.md) | `kube-system` | Pod Security Admission levels, default-deny ingress policies and image admission policies |
