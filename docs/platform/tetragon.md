...
| `kernel-module-load` | `security_kernel_module_request`, `security_kernel_read_file`, `find_module_sections` | A module requested by name or read from a container, minus the `net-pf-2-proto-262` request every Go binary makes when it probes MPTCP at start; the alert leaves out `rook-ceph`, whose CSI plugin loads `rbd` after a boot. The third hook records every load's signature check, host included: Flatcar signs its modules but does not enforce it (`CONFIG_MODULE_SIG` without `_FORCE`) | Yes: `TetragonKernelModuleLoaded`; `TetragonUnsignedKernelModule` from Loki |
| `dns-outside-cluster` | `ip_output` | A port 53 packet from a container to anything but the kube-dns ClusterIP, which is a hard-coded resolver in an image, a tunnel or exfiltration. The hook sees TCP and UDP alike, so musl's unconnected sends count too; the alert leaves out `kube-system`, where CoreDNS forwards upstream | Yes: `TetragonDnsOutsideCluster` |
