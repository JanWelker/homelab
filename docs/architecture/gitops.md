...
| `redis-ha.image.tag`, `redis-ha.haproxy.image.tag` | Both run ahead of the chart. The chart pins exact patch tags, and an exact patch tag stops being rebuilt once the next one lands, so it never picks up a base-image security update. `redis` 8.6.4-alpine was last built 2026-06-23 and therefore predates the Alpine OpenSSL 3.5.8-r0 of 2026-08-25. The override moved to 8.6.6-alpine, the head of that line, and Renovate has since carried it to 8.10.1-alpine — which is the point of listing it: the pin only stays current because something tracks it. Any 8.x above 8.2 is fine; the chart's own constraints are to avoid 7.4-8.0 for the licence change and 8.2.x for its Alpine 3.22 base. Renovate needs each bare `tag:` listed under the `pinDigests: false` rule in `renovate.json` |
